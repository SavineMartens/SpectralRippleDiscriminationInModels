...
def ax_colour_map_SMRT_per_kHz(ax, spike_rates_list, Fn_sel, sound_duration, y_axis_str='Frequency [kHz]', binsize=0.01, clim=[], norm=None, flim=None, octave_spaced = True):
    ...
    if octave_spaced:     
        ...
        ax.set_yticks([x_start, 0.5, 1, 2, 4, 8])
        ax.set_yticklabels([str(int(x_start)), '0.5', '1', '2', '4', '8'])
    return mesh, ax
...
if __name__ == '__main__':  
    ...
    octave_spaced = True
    dB = 65
    norm_bool = True # normalize total matrix
    if norm_bool:
        norm_bool_str = 'normalizedto1_'
    else:
        norm_bool_str = ''
    if dB == 50:
        data_dir = data_dir  + '/50dB_2416CF_nrep1/'
    elif dB == 65:
        data_dir = data_dir  + '/65dB_2416CF_nrep5/'

    if octave_spaced:
        octave_spaced_str = 'octave_spaced_'
    else:
        octave_spaced_str = 'linear_spaced_'
    ...
    if electric:
        ...
        for f_i, fname in enumerate(fname_list):
            ...
            if binsize_data != bin_size:
                spike_rates_list = rebin_spikes(spike_rates_list, binsize_data, bin_size) 
            ...
            if electric_scale == 'log_fft':
                ...
                spike_matrix = spike_rates_list[fiber_id_list,:]
            if norm_bool:
                spike_matrix = spike_matrix / np.max(spike_matrix) # normalize per fiber
            print(np.max(spike_matrix))
            RPO = fname[fname.index('width_')+len('width_'):]
            ...
            # assess presence vertical lines
            # plt.figure() 
            # for i in range(5): 
            #     plt.plot(np.arange(bin_size, sound_duration+bin_size, bin_size), spike_rates_list[2036+i,:]) # 2036 = 4000 Hz
            if 'p' in RPO:
                RPO.replace('p', '.')
            else:
                RPO + '.0'
        ...
        if cbar_bool:
            ...
            if norm_bool:
                fig.colorbar(mesh, cax=cbar_ax, label='Normalized spike rate')
            else:
            ...
            cbar_str = 'NoCbar'
        fig.savefig(save_dir + 'EH_cmap_' + cmap_type + '_clim_' + str(clim) + '_flim_' + str(flim) + '_binsize_' +str(bin_size) + '_norm_' + str(norm) + norm_bool_str + cbar_str +'_'+ electric_scale +'2416Fib111.6dB' + octave_spaced_str + '.png')
    ...
    if normal:
        ...
        if dB == 65:
            ...
            if num_fibers == 2416:
                fname_list = ['PSTH_filter2025-05-23_02-43-06_SMRT_stimuli_C_dens_33_rate_5_depth_20_width_4_2416CFs',
                              'PSTH_filter2025-05-23_02-40-43_SMRT_stimuli_C_dens_100_rate_5_depth_20_width_4_2416CFs',
                              'PSTH_filter2025-05-22_19-20-34_SMRT_stimuli_C_dens_33_rate_5_depth_20_width_16_2416CFs',
                              'PSTH_filter2025-05-23_01-13-58_SMRT_stimuli_C_dens_100_rate_5_depth_20_width_16_2416CFs']
                nrep=5
        if dB == 50: 
            if num_fibers == 2847:
        ...
        # clim=(2000, None)
        clim = []
        norm= None #matplotlib.colors.PowerNorm(.999) #None # matplotlib.colors.Normalize()
        sound_duration = 0.5
        # flim=None
        ...
        for f_i, fname in enumerate(fname_list):
            ...
            spike_rates_list = rebin_spikes(spikes_unfiltered, binsize_unfiltered, bin_size)/bin_size /50/nrep
            if norm_bool:
                spike_rates_list = spike_rates_list / np.max(spike_rates_list) 
        ...
        if cbar_bool:
            ...
            cbar_str = 'NoCbar'
        fig.savefig(save_dir + 'NH_RPO4vs16_both_density_cmap_' + cmap_type + '_clim_' + str(clim) + '_flim_' + str(flim) + '_binsize_' + str(bin_size) + '_norm_' + str(norm_str) + norm_bool_str +  'NoTitle'+ cbar_str+ str(dB)+'dB' + str(num_fibers) + 'NumFib' + octave_spaced_str + '.png')
